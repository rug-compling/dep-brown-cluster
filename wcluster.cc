/*
Hierarchically clusters phrases. Extension using dependency parses.
Comments indicating any changes are introduced by "dlm"
...
IntVec phrase_freqs;     //dlm: phrase a < N -> number of times heads and dependents appear in the text
IntVecVec head_phrases;  //dlm: phrase a < N -> list of phrases that appear as heads of a in the text
IntVecVec dep_phrases; //dlm: phrase a < N -> list of phrases that appear as dependents a in the text
...
IntVecVec tuples; //dep: instead of intvec text

void read_text_process_word(IntVec tuple) { //dlm: pushing back tuples instead of words
  tuples.push_back(tuple); //dlm
}
void read_text() {
  track("read_text()", "", false);
  read_text(text_file.c_str(), read_text_process_word, db, !use_restrict, !use_restrict, !use_restrict);
  //forvec(, IntVec, l, tuples) {
  
  int tuples_len = len(tuples); //dlm: not the same as T

  //dlm: count tokens using counts from input file
  foridx(i, tuples_len){ //dlm
    T += tuples[i].back(); //dlm
  }
  ...
  track_block("Counting phrases", "", false) {
    ...
    for(int l = 1; l <= plen; l++) {
      ...
      for(int i = 0; i < tuples_len-l+1; i++) { //dlm: read lines
	IntVec a_vec; //dlm: head
	IntVec b_vec; //dlm: dep
	a_vec.push_back(tuples[i][0]); //dlm
	b_vec.push_back(tuples[i][1]); //dlm
       	int count = tuples[i][2]; //dlm

	if(is_good_phrase(a_vec)) freqs[a_vec] += count;//dlm
	if(is_good_phrase(b_vec)) freqs[b_vec] += count;//dlm
      }
  ...
  track_block("Finding head/dep phrases", "", false) { //dlm: "left" is "head", "right" is "dep"
    head_phrases.resize(N);//dlm: not really the same in dlm
    dep_phrases.resize(N); //dlm
    for(int l = 1; l <= plen; l++) {
      for(int i = 0; i < tuples_len-l+1; i++) { //dlm: i over tuples
	IntVec a_vec; //dlm
	IntVec b_vec; //dlm
	a_vec.push_back(tuples[i][0]); //dlm
	b_vec.push_back(tuples[i][1]); //dlm
	...
	if(!contains(vec2phrase, a_vec) || !contains(vec2phrase, b_vec)) continue; //dlm
        int a = vec2phrase[a_vec];//dlm: head
	int b = vec2phrase[b_vec];//dlm: dep
	//dlm: deps of a
	for(int c = 0; c < count; c++) //dlm: lazy solution
	  dep_phrases[a].push_back(b);//dlm

        //dlm: heads of b
	for(int c = 0; c < count; c++)//dlm
	  head_phrases[b].push_back(a);//dlm
  ...
  if(!featvec_file.empty()) {
    ...
    foridx(a, N) {
      ...
      add_to_set(head_phrases[a], phrase_counts, 0); //dlm
      add_to_set(dep_phrases[a], phrase_counts, N); //dlm
  ...
  foridx(a, N) {
    ...
    forvec(_, int, b, head_phrases[a]) //dlm
      logs("LEFT " << Cluster(b));
    forvec(_, int, b, dep_phrases[a]) //dlm
...
void create_initial_clusters() {
  ...
  FOR_SLOT(s) {
    int a = slot2cluster[s];
    IntIntMap dep_phrase_freqs; //dlm

    // Find collocations of (a, b), where both are clusters.
    forvec(_, int, b, dep_phrases[a])
      if(contains(cluster2slot, b))
        dep_phrase_freqs[b]++; //dlm

    forcmap(int, b, int, count, IntIntMap, dep_phrase_freqs) { //dlm
...
int main(int argc, char *argv[]) {
  ...
  track_mem(head_phrases); //dlm
  track_mem(dep_phrases); //dlm
